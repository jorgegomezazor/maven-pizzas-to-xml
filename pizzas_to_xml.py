...
def load(diccs,order_details, orders, pizza_types):
    root = ET.Element('Maven_Pizzas')  #creo el elemento Maven_Pizzas
    for i in range(len(diccs)):
    ...
    for o in order_details.columns:
        ...
        order.set('categoria', str(o))  #nombre de la categoría
        order.text = str(order_details[o].dtype) #tipo de dato
    for o in orders.columns:
        order = ET.SubElement(root, 'categorias') #creo el subelemento categorias
        order.set('categoria', str(o)) #nombre de la categoría
        order.text = str(orders[o].dtype) #tipo de dato
    for o in pizza_types:
        order = ET.SubElement(root, 'categorias') #creo el subelemento categorias
        order.set('categoria', str(o)) #nombre de la categoría
        order.text = str(pizza_types[o].dtype) #guardo los tipos de datos de cada columna
